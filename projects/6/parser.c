...
void assemble(const char * file_name, instruction* instructions, int num_instructions){
	char *hack_file_name = malloc(strlen(file_name) + 5);
	...
	for(int i = 0; i < num_instructions; i++){ 
		...
		if(instr.type == 'A' && instr.inst.a.is_addr == false){
			struct Symbol *symbol = symtable_find(instr.inst.a.operand.label);
			if(symbol == NULL){
				symtable_insert(instr.inst.a.operand.label, j);
				opcode = j;
				j++;
			}
			else{
			...
			free(instr.inst.a.operand.label);
			}
			else if(instr.type == 'A'){
				opcode = instr.inst.a.operand.address;
			}
			else if(instr.type == 'C'){
				opcode = instruction_to_opcode(instr.inst.c);
			}
